...
You have cluster admin access to a live OpenShift cluster. Let's make sure everything's working.

Once you have logged in, open the Web Terminal using the icon shown on the top right of the portal.

image::web-terminal.png[]

This will open the web terminal window at the bottom of the web page. Click the Start button to launch.

image::web-terminal-2.png[]

You can now use this terminal to enter any of the `oc` command used in the workshop.

image::web-terminal-3.png[]


Click the commands below:
...
This workshop lets you answer those questions through hands-on exercises. You'll break things and fix them, diagnose real failure modes, and see what OpenShift looks like when things go wrong (and how fast you can recover).
...
image::ocp-console-home.png[OpenShift Administrator Console Overview, link=self, window=blank]

You can open the console in a separate browser tab by clicking the URL:
...
In the Core perspective, you'll see the **Overview** dashboard under **Home → Overview**.

// image::ocp-dashboard-overview.png[OpenShift Administrator Dashboard Overview, link=self, window=blank]
image::ocp-console-home.png[OpenShift Administrator Dashboard Overview, link=self, window=blank]
...
image::observe-cluster-metrics-dashboard-2.png[Kubernetes Cluster Metrics Dashboard, link=self, window=blank]
...
image::nodes-console-view-2.png[Nodes Console View, link=self, window=blank]
...
image::machinesets-console-2.png[MachineSets view showing worker MachineSet with 0 of 0 machines on compact cluster, link=self, window=blank]
...
oc scale machineset.machine.openshift.io workshop-hg8v8-worker-ap-southeast-2a --replicas=1 -n openshift-machine-api
----
...
image::cluster-settings-details-2.png[Cluster Settings Details Tab, link=self, window=blank]
+
...
image::cluster-upgrade-versions-2.png[Available Upgrade Versions,width=600, link=self, window=blank]
...
image::software-catalog-operators-2.png[Software Catalog filtered to Operators showing available operator tiles, link=self, window=blank]
