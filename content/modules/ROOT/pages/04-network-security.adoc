...
OpenShift uses **OVN-Kubernetes** as its network plugin (the only supported option in 4.22). It handles pod networking, network isolation via NetworkPolicy, egress traffic control, and optional IPsec encryption.
...
[source,bash,role="copypaste"]
----
bash <(curl -sL https://raw.githubusercontent.com/finhennessy/workshops/main/support/04-network-security/test-dns.sh)
----
...
* **NetworkPolicy:** link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/networking/network-policy[About network policy, window=blank]
* **EgressFirewall:** link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/networking/configuring-egress-firewall-ovn[Configuring an egress firewall for a project, window=blank]
* **EgressIP:** link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/networking/configuring-egress-ips-ovn[Configuring an egress IP address, window=blank]
